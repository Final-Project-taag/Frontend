...
function Navbar({isAdmin, setShowNavbar, handleRegisterClick}) {
  ...
  const handleThemeSwitch = () => {
    ...
    console.log(setTheme(theme === "dark" ? "light" : "dark"))
    setIsDark(!isDark)
  }
  ...
  const userNavigation = [{title: " Buchungen", url: "/reservation-view"}]
  const adminNavigation = [
    {title: "Fahrzuege hinzufügen", url: "/admin-view"},
      ...
      <div className=" md:flex items-center justify-center md:justify-end w-fit  md:mr-0  ">
        {!isAuthenticated ? (
          ...
          <div className="flex  text-3xl  w-full justify-center items-center gap-4 py-2 ">
            <Icon text={isAuthenticated && authStore.user.username}>
              {" "}
              <HiOutlineUserCircle color={!isDark ? " #475569" : "#94a3b8"} size={36} />{" "}
            </Icon>
            <Icon text="logout" action={() => authStore.logout()}>
              {" "}
              <IoLogOutOutline color={!isDark ? " #475569" : "#94a3b8"} size={36} />{" "}
            </Icon>
            <Theme handleThemeSwitch={handleThemeSwitch} isDark={isDark} />
          </div>
        )}
          ...
          ? user?.role?.name === "admin"
            ? adminNavigation.map(item => (
                <li
                  onClick={closeNavbar}
                  key={item.title}
                  className="  font-base     lg: text-xl text-green-600 dark:text-green-500 hover:text-gray-600 "
                >
                  <Link to={item.url}>{item.title}</Link>
                </li>
              ))
